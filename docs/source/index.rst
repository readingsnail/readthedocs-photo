...
사진이란 뭘까?
-----------
사진의 한자 뜻(寫眞)인 진짜를 베끼다(자세한 것은 `아우라 <https://photo-technic-tmi.readthedocs.io/ko/latest/기타.html#id4>`_ 항목을 보세요)보단, 이게 더 낫겠군요 - 위키백과 등에선 ‘빛, 즉 광자를 기록’ 하는 걸 말합니다. 영어의 Photography도 같구요. 광자를 기록한다로 정의하면 엄청나게 범위가 넓어집니다. 현 반도체 생산 공정인 리소그래피(Lithography)도 광자(현재는 X선쪽에 약간 포함되는 EUV)를 실리콘에 ‘노출’ 시켜 ‘인화’ 하는 과정을 반복하여 칩을 제작합니다.

네, 생각도 하지 않았던 것들이 사진 범위에 포함되지만, 우린 그것들을 사진술이라 부르지 않습니다.

아마 제가 좋아하는 정줄을 제대로 보내버린 판타지[Discworld]에 등장하는 사진과 사진술이 이 정의에 맞을거 같군요. 이 세계 최초의 관광객(이자 가이드 피셜 머저리)인 두송이꽃(Twoflower)이 Iconograph 란 상자를 들고 옵니다. 임프를 사용하여 빨리 그리는 그림 상자였죠.

.. image:: images/otto.jpg
 :width: 600

이후 두송이꽃이 가져온 그림 상자의 명칭을 따서 사진기와 사진술이 퍼졌고, 이런 빛을 그려내는 것에 삶과 죽음을 왔다갔다 해도 좋아하는 윗 그림의 앙크 모포크 타임즈 수석 사진기자인 뱀파이어 오토 크리엑(Otto Chriek)도 있죠. 문제는 뱀파이어라는 점과 더불어 샐러맨더 플래시를 터트리면 재로 변해버리는 터라 누가 피를 붓지 않으면(나중엔 깨지기 쉬운 유리병 목걸이에 동물 피를 넣어 다니는) 시체 아니 그냥 재니까 말이죠. 그렇지만, 그렇게 되어도 사진기와 빛에 열광하죠.
